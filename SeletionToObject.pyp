...
import c4d
import os
from c4d import documents, utils,Vector,plugins,bitmaps
...
#Transform texture projection to UVW  -> Credits to César Vonc: https://code.vonc.fr/?a=20
def createUVW(obj):
...
def proceedObject(obj,progress,numAllTextures):
    ...
    for i in reversed(tags):
        if i.CheckType(c4d.Ttexture):
            progress+=1
            c4d.StatusSetText("Exporting "+str(progress)+"of "+str(numAllTextures))
            c4d.StatusSetBar(100.0*progress/numAllTextures) 
            if i[c4d.TEXTURETAG_RESTRICTION]!="": 
                sel =i[c4d.TEXTURETAG_RESTRICTION]
            else: sel = None            
            if sel==None:
                newObjs.append( copyObject (obj,i))
            elif sel!=None and checkAlpha(i)==True and checkEmpty(i)==False:
...
class SelectionToObject(plugins.CommandData):
